...
class DioExceptions implements Exception {
  DioExceptions.fromDioError(DioException dioError) {
    switch (dioError.type) {
      ...
      case DioExceptionType.badResponse:
        ...
        message = _handleError(dioError.response?.statusCode, dioError.response?.statusMessage);
        break;
      case DioExceptionType.unknown:
        if (dioError.error is SocketException || dioError.error is HandshakeException) {
          message = "Unable to connect. Please check that you are connected to "
              "the internet and try again.";
          break;
        }
        message =
            dioError.message?.isEmpty ?? true ? "Error retrieving information" : dioError.message;
        break;
    }
  }
  ...
  @override
  String toString() => message ?? "Unexpected error occurred!";
}
